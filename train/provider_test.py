...
def get_pixels(index):
    pixel_dir = "/home/amben/frustum-pointnets_RSC/dataset/KITTI/object/training/pc_to_pixels/"
...
def get_closest_pc_to_center(pc,pixels,center_box2d):
    print(np.amin(np.linalg.norm(pixels-center_box2d)))
    idx = np.argmin(np.linalg.norm(pixels-center_box2d,axis=1))
    print(idx)
    center3d = pc[idx,:]
    return center3d
def get_2Dboxes_detected(idx):
    det_2dboxes_path = "/home/amben/frustum-pointnets_RSC_2D/dataset/RSC/labelsVal2D/"
        ...
        if from_rgb_detection:
            ...
            for i in range(len(self.id_list)):
                ...
                for j in range(len(gt_obj_list)):
                    for _ in range(augmentX):
                        ...
                        gt_boxes3d = kitti_utils.objs_to_boxes3d([gt_obj_list[j]])
                        ...
                        self.size_list.append(np.array([gt_obj_list[j].l, gt_obj_list[j].w, gt_obj_list[j].h]))
            ...
            for i in range(len(self.id_list)):
                ...
                for j in range(len(box2ds)):
                    ...
                    print("average number of cloud:",count/ len(self.input_list))
                    self.frustum_angle_list.append(frustum_angle)
