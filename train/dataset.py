...
class KittiDataset():
    def __init__(self, root_dir,dataset='KITTI', split='train', mode='TRAIN'):
        self.split = split
        self.mode = mode
        ...
        self.label_dir_2D = os.path.join(self.imageset_dir, 'lables2D_rect_all_hand')
        self.label_dir = os.path.join(self.imageset_dir, 'label_2_kitti2')

        self.plane_dir = os.path.join(self.imageset_dir, 'planes')
        self.radar_dir = os.path.join(self.imageset_dir, 'pc_radar_2')
    ...
    def get_label_2D(self, idx):

        label_file = os.path.join(self.label_dir_2D, '%06d.txt' % idx)
        #print(label_file)
